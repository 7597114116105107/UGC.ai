      ...
      <div className="bg-white/5 border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
       ...
       <div className={`${gen?.aspectRatio === '9:16' ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
         ...
         {(!gen?.generatedImage && !gen?.generatedVideo) && (
          <div className="absoolute inset-0 w-full h-full flex flex-col items-center justify-center bg-black/20">
            <Loader2Icon className="size-7 animate-spin" />
          </div>
         )}

         {/* status badges */}
         <div className="absoluete left-3 top-3 flex gap-2 items-center">
          {gen.isGenerating && (
       ...
       <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            {gen.updateAt && (
              <p>Updated: {new Date(gen.updateAt).toLocaleString()}</p>
            )}
          </div>
        ...
        {gen.productDescription && (
          <div className="mt-3">
            ...
            <div className="text-sm text0gray-300 bg-white/3 p-2 rounded-md wrap-break-word">{gen.productDescription}</div>
          </div>
        )}
