...
const UploadZone = ({label, file, onclear, onChange}: UploadZoneProps) => {
  return (
    <div className="relative group">
      <div className={`relative h-60 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center bg-white/2 p-6 ${file ? 'border-violet-600/50 bg-violet-500/5' : 'border-white/10 hover:border-violet-500/30 hover:bg-white/5'}`}>
      {file ? (
        <>
        ...
        <div className="absolute bottom-4 left-4 right bg-black/50 backdrop-blur-md p-3 rounded-lg border border-white/10">
        ...
        <h3 className="text-lg font-semmibold mb-2">{label}</h3>
